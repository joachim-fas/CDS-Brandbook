...
import { User, ShoppingBag, Search, Menu, X, ArrowRight, Heart, Tag, Download, FolderDown, FileText } from 'lucide-react';
...
export function AssetLibrary({ themeColors, currentHex, handleCopy }: AssetLibraryProps) {
  ...
  const generateBrandbookPdf = () => {
    const doc = new jsPDF();
    
    doc.setFontSize(20);
    doc.text('Culturedrops Brandbook Summary', 20, 20);
    
    doc.setFontSize(12);
    doc.text('01. Brand DNA', 20, 40);
    doc.text('- Uncompromising Aesthetics: Stripped back to essentials.', 25, 50);
    doc.text('- Collective Identity: Designed to be worn together.', 25, 60);
    doc.text('- Zeitgeist Captured: Distilling contemporary movements.', 25, 70);
    
    doc.text('02. Color System', 20, 90);
    doc.text('Core Neon Palette: Acid (#00FF00), Hyper (#00E5FF), Synth (#B200FF), Volt (#CCFF00).', 25, 100);
    doc.text('Base Colors: Asphalt (#111111), Pure (#FFFFFF), BgLight (#F4F4F0).', 25, 110);
    
    doc.text('03. Typography', 20, 130);
    doc.text('- Headline: Syne (Regular, Bold, ExtraBold)', 25, 140);
    doc.text('- Body/Mono: Space Mono (Regular, Bold)', 25, 150);
    
    doc.text('04. UI & UX Elements', 20, 170);
    doc.text('- Brutalist form fields, custom checkboxes, and button sandbox.', 25, 180);
    
    doc.text('05. Patterns & Animations', 20, 200);
    doc.text('- Lined Notepad, Street Checker, Caution Tape, Core Glitch.', 25, 210);
    
    doc.save('culturedrops-brandbook-summary.pdf');
    handleCopy(' ', 'Downloading Brandbook Summary PDF...');
  };
      ...
      const getSvg = (hex: string, withShadow: boolean = false) => {
        ...
        if (withShadow) {
          ...
          if (innerContentMatch && svgTagMatch) {
            ...
            return `${svgTag}
              <g transform="translate(4, 4)" color="#111111">${innerContent}</g>
  ...
  const downloadAsset = (Component: any, filename: string, format: 'svg' | 'png', colorHex: string = '#111111', withShadow: boolean = false, props: any = {}) => {
    let svgString = renderToStaticMarkup(<Component {...props} />);
    ...
    if (withShadow && colorHex !== '#111111') {
      ...
      if (innerContentMatch && svgTagMatch) {
        ...
        svgString = `${svgTag}
          <g transform="translate(4, 4)" color="#111111">${innerContent}</g>
          <g color="${colorHex}">${innerContent}</g>
        </svg>`;
      } else {
  ...
  return (
    <div className="pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <h1 className="font-headline text-5xl md:text-6xl text-asphalt mb-6 uppercase">Asset Library</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-6 mb-20">
        <button 
          ...
          className="bg-brand text-asphalt font-headline tracking-widest text-lg px-8 py-5 font-bold uppercase border-4 border-asphalt shadow-[6px_6px_0_0_#111111] hover:translate-y-1 hover:shadow-[2px_2px_0_0_#111111] transition-all flex items-center justify-center gap-3"
        >
          <FolderDown size={24} />
      ...
      <section className="mb-20">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b-2 border-asphalt/10 pb-4 gap-4">
          <h2 className="font-headline text-3xl text-asphalt">Logos</h2>
          <button 
            onClick={() => setShowShadow(!showShadow)}
            className="text-sm font-bold uppercase tracking-widest bg-asphalt text-pure px-4 py-2 hover:bg-brand hover:text-asphalt transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-pure border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-asphalt/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-black-flat`, 'svg', '#111111', false)} className="bg-brand text-asphalt font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-black-flat`, 'png', '#111111', false)} className="bg-pure text-asphalt font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
          ...
          <div className="bg-asphalt border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-pure/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-white-${showShadow ? 'shadow' : 'flat'}`, 'svg', '#FFFFFF', showShadow)} className="bg-brand text-asphalt font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-white-${showShadow ? 'shadow' : 'flat'}`, 'png', '#FFFFFF', showShadow)} className="bg-asphalt text-pure font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> PNG
              </button>
            </div>
          ...
          <div className="bg-pure border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-asphalt/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-acid-${showShadow ? 'shadow' : 'flat'}`, 'svg', '#00FF00', showShadow)} className="bg-[#00FF00] text-asphalt font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-acid-${showShadow ? 'shadow' : 'flat'}`, 'png', '#00FF00', showShadow)} className="bg-pure text-asphalt font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> PNG
              </button>
            </div>
          ...
          <div className="bg-pure border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-asphalt/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-hyper-${showShadow ? 'shadow' : 'flat'}`, 'svg', '#00E5FF', showShadow)} className="bg-[#00E5FF] text-asphalt font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-hyper-${showShadow ? 'shadow' : 'flat'}`, 'png', '#00E5FF', showShadow)} className="bg-pure text-asphalt font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> PNG
              </button>
            </div>
          ...
          <div className="bg-pure border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-asphalt/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-synth-${showShadow ? 'shadow' : 'flat'}`, 'svg', '#B200FF', showShadow)} className="bg-[#B200FF] text-pure font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-synth-${showShadow ? 'shadow' : 'flat'}`, 'png', '#B200FF', showShadow)} className="bg-pure text-asphalt font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> PNG
              </button>
            </div>
          ...
          <div className="bg-pure border-2 border-asphalt p-6 shadow-brutal-black flex flex-col items-center justify-center relative group">
            ...
            <div className="absolute inset-0 bg-asphalt/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-volt-${showShadow ? 'shadow' : 'flat'}`, 'svg', '#CCFF00', showShadow)} className="bg-[#CCFF00] text-asphalt font-bold px-4 py-2 border-2 border-asphalt mb-2 hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> SVG
              </button>
              <button onClick={() => downloadAsset(LogoSVG, `culturedrops-logo-volt-${showShadow ? 'shadow' : 'flat'}`, 'png', '#CCFF00', showShadow)} className="bg-pure text-asphalt font-bold px-4 py-2 border-2 border-asphalt hover:scale-105 transition-transform flex items-center gap-2">
                <Download size={16} /> PNG
              </button>
            </div>
      ...
      {/* Typography / Fonts */}
      <section className="mb-20">
        <h2 className="font-headline text-3xl text-asphalt mb-8 border-b-2 border-asphalt/10 pb-4">Typography</h2>
      ...
      {/* Design Tokens */}
      <section className="mb-20">
        <h2 className="font-headline text-3xl text-asphalt mb-8 border-b-2 border-asphalt/10 pb-4">Design Tokens</h2>
  ...
  "typography": {
    ...
    "weights": {
      ...
      {/* Icons */}
      <section className="mb-20">
        <h2 className="font-headline text-3xl text-asphalt mb-8 border-b-2 border-asphalt/10 pb-4">Icon Set (SVG)</h2>
            ...
            <div key={label} className="bg-pure border-2 border-asphalt p-4 group hover:bg-brand hover:-translate-y-1 shadow-[2px_2px_0_0_#111111] hover:shadow-[4px_4px_0_0_#111111] transition-all cursor-pointer relative">
              ...
              <div className="absolute inset-0 bg-asphalt/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <button 
                  ...
                  className="text-brand font-bold text-xs flex flex-col items-center gap-1 hover:scale-110 transition-transform"
                >
                  <Download size={16} /> SVG
                </button>
              </div>
      ...
      <section className="mb-20">
        <h2 className="font-headline text-3xl text-asphalt mb-8 border-b-2 border-asphalt/10 pb-4">Patterns & Animations</h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
...
@keyframes glitch-anim {
  ...
  /* ... more keyframes ... */
        ...
        <div className="mt-12 flex justify-center">
          <button 
            onClick={generateBrandbookPdf}
            className="bg-pure text-asphalt font-headline tracking-widest text-lg px-8 py-5 font-bold uppercase border-4 border-asphalt shadow-[6px_6px_0_0_#111111] hover:translate-y-1 hover:shadow-[2px_2px_0_0_#111111] transition-all flex items-center justify-center gap-3"
          >
            <FileText size={24} />
